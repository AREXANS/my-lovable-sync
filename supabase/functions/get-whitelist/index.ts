...
serve(async (req) => {
  ...
  try {
    ...
    const { data: manualData } = await supabase
      .from("app_settings").select("value").eq("key", "manual_whitelist").maybeSingle();

    const { data: blacklistData } = await supabase
      .from("app_settings").select("value").eq("key", "blacklist_usernames").maybeSingle();

    const blacklisted = new Set<string>();
    if (blacklistData?.value) {
      try {
        const list = JSON.parse(blacklistData.value);
        if (Array.isArray(list)) {
          for (const b of list) {
            const name = typeof b === "string" ? b : b?.username;
            if (name) blacklisted.add(String(name).toLowerCase());
          }
        }
      } catch { /* ignore */ }
    }
      ...
      try {
        ...
        if (Array.isArray(manualUsers)) {
          for (const user of manualUsers) {
            if (user.username && !usernameSet.has(user.username.toLowerCase()) && !blacklisted.has(user.username.toLowerCase())) {
