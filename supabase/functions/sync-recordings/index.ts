...
function sanitize(record: Record<string, unknown>, requesterKey?: string | null, likedIds?: Set<string>) {
  return {
    id: record.id,
    title: record.title,
    ...
    liked: Boolean(likedIds && likedIds.has(String(record.id))),
    owned: Boolean(requesterKey && record.owner_key === requesterKey),
  };
}

async function recountLikes(supabase: ReturnType<typeof createClient>, recordingId: string) {
  const { count } = await supabase
    .from("lua_recording_likes")
    .select("id", { count: "exact", head: true })
    .eq("recording_id", recordingId);
  const total = Number(count || 0);
  await supabase.from("lua_recordings").update({ likes: total }).eq("id", recordingId);
  return total;
}
...
serve(async (req) => {
  ...
  try {
    ...
    if (req.method === "GET") {
      ...
      if (key) {
        const { data: likeRows } = await supabase
          ...
          .select("recording_id")
          .eq("liker_key", key);
        likedIds = new Set((likeRows || []).map((r) => String((r as any).recording_id)));
      }

      return new Response(JSON.stringify({ success: true, recordings: (data || []).map((r) => sanitize(r, key, likedIds)) }), {
        headers: jsonHeaders,
      });
    }
    ...
    // Public like/unlike/toggle — identitas bisa AXS key atau device id dari web
    if (action === "like" || action === "unlike" || action === "toggle_like") {
      const id = String(body.id || "");
      const liker = String(body.likerId || body.liker || key || "").trim().slice(0, 180);
      if (!id) return new Response(JSON.stringify({ success: false, error: "ID wajib diisi" }), { status: 400, headers: jsonHeaders });
      if (!liker) return new Response(JSON.stringify({ success: false, error: "Identitas liker wajib diisi" }), { status: 400, headers: jsonHeaders });
      ...
      const shouldLike = action === "like" ? true : action === "unlike" ? false : !existing;

      if (shouldLike && !existing) {
        const { error } = await supabase.from("lua_recording_likes").insert({ recording_id: id, liker_key: liker });
        if (error) throw error;
      } else if (!shouldLike && existing) {
        const { error } = await supabase.from("lua_recording_likes").delete().eq("id", (existing as any).id);
        if (error) throw error;
      }

      const total = await recountLikes(supabase, id);
      return new Response(JSON.stringify({ success: true, liked: shouldLike, likes: total }), { headers: jsonHeaders });
    }
    ...
    if (action === "delete" || action === "remove") {
      ...
      if (!deletedRows || deletedRows.length === 0) {
        return new Response(JSON.stringify({ success: false, error: "Rekaman tidak ditemukan atau bukan pemilik" }), { status: 404, headers: jsonHeaders });
      }
      return new Response(JSON.stringify({ success: true, deleted: deletedRows.length }), { headers: jsonHeaders });
    }
