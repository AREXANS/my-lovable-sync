...
const OrderForm: FC<OrderFormProps> = ({
  ...
  const roleClass = selectedPkg === 'LIFETIME' ? 'role-lifetime' : selectedPkg === 'VIP' ? 'role-vip' : 'role-normal';
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden">
      <GlobalBackground />
      <div className={`role-card ${roleClass} p-8 rounded-2xl max-w-md w-full relative shadow-2xl z-10 animate-scale-in`}>
        <button onClick={onBack} className="absolute top-6 left-6 text-muted-foreground hover:text-foreground transition-colors duration-200 flex items-center gap-2 font-medium z-20">
          <span>←</span> Kembali
        </button>

        <div className="text-center mb-8 pt-8">
          <div className="role-badge inline-block px-4 py-2 rounded-full text-sm font-bold mb-4 animate-pulse-slow">
            Paket {selectedPkg === 'LIFETIME' ? 'LIFETIME ADMIN' : selectedPkg}
          </div>
          <h2 className="text-2xl font-display font-bold role-text">Isi Data Pembelian</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {/* LIFETIME info */}
          {isLifetime && (
            <div className="role-panel p-4 animate-slide-in">
              <div className="flex justify-between mb-2">
                <span className="text-muted-foreground">Paket:</span>
                <span className="font-bold role-text">LIFETIME ADMIN</span>
              </div>
              <div className="flex justify-between mb-2">
              ...
              <div className="flex justify-between mb-2">
                ...
                <span className="font-bold role-text">ADMIN</span>
              </div>
              <div className="border-t pt-2 mt-2" style={{ borderColor: 'hsl(var(--r1) / 0.3)' }}>
                <div className="flex justify-between">
                  <span className="font-semibold text-foreground">Total:</span>
                  <span className="font-bold role-text">{formatRupiah(lifetimePrice)}</span>
          ...
          {durationData && (
            <div className="bg-muted/50 p-4 rounded-xl border border-border animate-slide-in">
              ...
              <div className="border-t border-border pt-2 mt-2">
                <div className="flex justify-between">
                  <span className="font-semibold text-foreground">Total:</span>
                  <span className="font-bold role-text">{formatRupiah(finalTotal)}</span>
                </div>
              </div>
            </div>
          )}
          ...
          <Button type="submit" disabled={loading || xcoinsPayLoading || ((paymentMethod === 'xcoins' || xcoinsOnly) && !xcoinsUser)} className="role-btn w-full py-6 font-display font-bold text-lg">
