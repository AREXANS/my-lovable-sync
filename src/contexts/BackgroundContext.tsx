...
interface BackgroundContextType {
  ...
  showBackground: (background: Background) => void;
}
...
export const BackgroundProvider = ({ children }: { children: ReactNode }) => {
  ...
  const fetchBackgrounds = async () => {
    try {
      ...
      if (data && data.length > 0) {
        ...
        setCurrentIndex(index => Math.min(index, data.length - 1));
      } else {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel('backgrounds-live')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'backgrounds' }, () => {
        fetchBackgrounds();
      })
      .subscribe();

    const onFocus = () => fetchBackgrounds();
    window.addEventListener('focus', onFocus);

    // Fallback polling agar perubahan background tetap terdeteksi tanpa realtime
    const poll = setInterval(fetchBackgrounds, 20000);
  ...
  const showBackground = useCallback((background: Background) => {
    setActiveBackgrounds(previous => [
      background,
      ...previous.filter(item => item.id !== background.id),
    ]);
    setCurrentIndex(0);
    setLoading(false);
  }, []);
