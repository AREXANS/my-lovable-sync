...
// Key pembelian dianggap expired kalau tanggal expired-nya sudah lewat
const isKeyExpired = (tx: { expires_at?: string | null; status: string }) => {
              ...
              filtered.map((tx) => (
                <Card
                  ...
                  className={`glass-card overflow-hidden ${
                    isKeyExpired(tx)
                      ? 'border-warning/60 bg-warning/5 shadow-[0_0_18px_-6px_hsl(var(--warning)/0.55)]'
                      : 'border-border/50'
                  }`}
                >
                  <CardContent className="p-4 space-y-3">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <Package className={`w-4 h-4 ${isKeyExpired(tx) ? 'text-warning' : 'text-primary'}`} />
                        <span className={`font-display font-semibold text-sm ${isKeyExpired(tx) ? 'text-warning' : 'text-foreground'}`}>{tx.package_name}</span>
                      ...
                      <div className="flex items-center gap-1.5">
                        {isKeyExpired(tx) && tx.status !== 'expired' && (
                          <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full border border-warning/40 bg-warning/20 text-warning">
                            Key Expired
                          </span>
                        )}
